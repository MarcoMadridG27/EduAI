...
def generate_sesion(data: dict, output_path: str = "sesion_aprendizaje.pdf"):
    ...
    def v(key, default=""):
        ...
        if val is None or val == "":
            return str(default)
        return str(val)  # Garantiza siempre string aunque llegue int/float
    ...
    col_w = [2.4*cm, W_PAGE/2-2.4*cm, 2.4*cm, W_PAGE/2-2.4*cm]
    ...
    for el in momento_block("CIERRE", colors.HexColor("#0D47A1"), cierre_data, cierre_dur):
        story.append(el)
...
if __name__ == "__main__":
    generate_sesion(EJEMPLO, "/mnt/user-data/outputs/sesion_aprendizaje_template.pdf")
